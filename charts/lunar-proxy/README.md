...
```bash
helm install lunar-proxy lunar/lunar-proxy --version v1.1.0-alpha-90edcf2 --set tenantName=<organization-name> --set lunarAPIKey=<api-key>
```
...
```bash
helm install lunar-proxy lunar/lunar-proxy --version v1.1.0-alpha-90edcf2 -f ./values-override.yaml
```
...
spec:
  ...
  source:
    ...
    targetRevision: 'v1.1.0-alpha-90edcf2'
    chart: lunar-proxy
    helm:
      values: |
        logLevel: "INFO"
