...
```bash
helm install lunar-proxy lunar/lunar-proxy --version 0.10.25-fix1 --set tenantName=<organization-name> --set lunarAPIKey=<api-key>
```
...
```bash
helm install lunar-proxy lunar/lunar-proxy --version 0.10.25-fix1 -f ./values-override.yaml
```
