...
```bash
helm install lunar-proxy lunar/lunar-proxy --version v1.1.0-alpha-ae1883e --set tenantName=<organization-name> --set lunarAPIKey=<api-key>
```
...
```bash
helm install lunar-proxy lunar/lunar-proxy --version v1.1.0-alpha-ae1883e -f ./values-override.yaml
```
...
spec:
  ...
  source:
    ...
    targetRevision: 'v1.1.0-alpha-ae1883e'
    chart: lunar-proxy
    helm:
      values: |
        logLevel: "INFO"
