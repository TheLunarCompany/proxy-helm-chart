...
```bash
helm install lunar-proxy lunar/lunar-proxy --version v1.1.6-alpha-6b85d1f --set tenantName=<organization-name> --set lunarAPIKey=<api-key>
```
...
```bash
helm install lunar-proxy lunar/lunar-proxy --version v1.1.6-alpha-6b85d1f -f ./values-override.yaml
```
...
spec:
  ...
  source:
    ...
    targetRevision: 'v1.1.6-alpha-6b85d1f'
    chart: lunar-proxy
    helm:
      values: |
        logLevel: "INFO"
