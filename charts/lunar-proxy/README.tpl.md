...
```bash
helm install lunar-proxy lunar/lunar-proxy --version $CHART_VERSION --set tenantName=<organization-name> --set lunarAPIKey=<api-key>
```
...
```bash
helm install lunar-proxy lunar/lunar-proxy --version $CHART_VERSION -f ./values-override.yaml
```
