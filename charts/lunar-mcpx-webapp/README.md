...
```bash
helm install mcpx lunar/lunar-mcpx-webapp --version 0.9.59 --set postgres.enabled=true --set redis.enabled=true
```

Alternatively, you may work with a separate values file to handle values override just like any other Helm chart:

```bash
helm install mcpx lunar/lunar-mcpx-webapp --version 0.9.59  -f ./values-override.yaml
```
